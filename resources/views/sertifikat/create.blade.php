<x-app-layout>
    <x-slot name="header"><h2 class="font-semibold text-xl text-gray-800">Tambah Sertifikat</h2></x-slot>
    <div class="py-8">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-sm rounded-xl p-6">
                <form method="POST" action="{{ route('sertifikat.store') }}" enctype="multipart/form-data" class="space-y-5">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Admin BLK</label>
                        <select name="admin_id" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 focus:outline-none">
                            <option value="">-- Pilih Admin --</option>
                            @foreach($admins as $a)
                            <option value="{{ $a->id }}" {{ old('admin_id') == $a->id ? 'selected' : '' }}>{{ $a->user->name ?? 'Admin #'.$a->id }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="nomor_sertifikat" value="{{ old('nomor_sertifikat') }}" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 focus:outline-none">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Tanggal Terbit</label>
                            <input type="date" name="tanggal_terbit" value="{{ old('tanggal_terbit') }}" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 focus:outline-none">
                        </div>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">File Sertifikat <span class="text-gray-400 text-xs">(PDF/JPG/PNG, maks 5MB)</span></label>
