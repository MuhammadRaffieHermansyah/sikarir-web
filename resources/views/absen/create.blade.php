...
@section('title', 'Catat Presensi Peserta - BLK')
  ...
  <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
    <div>
      <div class="flex items-center gap-2 text-xs text-slate-400 mb-1">
        ...
        <span class="text-slate-600 font-medium">Tambah Presensi</span>
      </div>
      <h1 class="text-xl font-black text-slate-900 tracking-tight">Catat Kehadiran Peserta</h1>
      <p class="text-xs text-slate-500 mt-0.5">Isi formulir di bawah untuk merekam presensi harian peserta pelatihan.</p>
    </div>
  ...
  <div class="max-w-3xl mx-auto">
    <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
      <div class="p-5 border-b border-slate-100 flex items-center gap-2.5 bg-slate-50/50">
        ...
        <div>
          <h2 class="font-bold text-slate-800 text-sm">Formulir Presensi Harian</h2>
          <p class="text-[11px] text-slate-500">Pastikan kombinasi peserta, jadwal, dan tanggal tidak duplikat.</p>
        </div>
      </div>

      <form method="POST" action="{{ route('absen.store') }}" class="p-6 space-y-5">
        @csrf
        ...
        @if($errors->any())
          <div class="p-4 bg-rose-50 border border-rose-200 rounded-xl text-xs text-rose-700 space-y-1">
            <div class="font-bold flex items-center gap-1.5">
              <i data-lucide="alert-circle" class="w-4 h-4"></i> Gagal menyimpan data:
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
          ...
          <div class="space-y-1">
            ...
            <input type="time" name="jam_hadir" value="{{ old('jam_hadir', date('H:i')) }}" class="w-full bg-white border @error('jam_hadir') border-rose-500 @else border-slate-200 @enderror rounded-lg px-3 py-2 text-xs font-medium text-slate-700 focus:outline-none focus:border-emerald-600 focus:ring-1 focus:ring-emerald-600 shadow-sm">
            @error('jam_hadir')
              <p class="text-[10px] text-rose-500 font-medium">{{ $message }}</p>
            @enderror
          </div>
          ...
          <div class="space-y-1">
            ...
            <select name="status_kehadiran" required class="w-full bg-white border @error('status_kehadiran') border-rose-500 @else border-slate-200 @enderror rounded-lg px-3 py-2 text-xs font-medium text-slate-700 focus:outline-none focus:border-emerald-600 focus:ring-1 focus:ring-emerald-600 shadow-sm">
              @foreach(['hadir' => 'Hadir', 'izin' => 'Izin', 'sakit' => 'Sakit', 'alpha' => 'Alpha'] as $key => $label)
                <option value="{{ $key }}" {{ old('status_kehadiran') === $key ? 'selected' : '' }}>{{ $label }}</option>
              @endforeach
            </select>
        ...
        <div class="space-y-1">
          ...
          <textarea name="keterangan" rows="3" placeholder="Tambahkan catatan jika izin, sakit, atau ada informasi penting..." class="w-full bg-white border border-slate-200 rounded-lg p-3 text-xs font-medium text-slate-700 focus:outline-none focus:border-emerald-600 focus:ring-1 focus:ring-emerald-600 shadow-sm">{{ old('keterangan') }}</textarea>
          @error('keterangan')
            <p class="text-[10px] text-rose-500 font-medium">{{ $message }}</p>
          @enderror
        </div>
