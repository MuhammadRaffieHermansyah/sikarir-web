  ...
  <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            ...
            <tr>
              <td colspan="6" class="px-5 py-10 text-center text-slate-400">
                <div class="flex flex-col items-center justify-center gap-2">
                  ...
                  <p class="text-xs font-medium text-slate-600">Belum ada data absensi yang ditemukan</p>
                  @if(request('search') || request('status'))
                    <a href="{{ route('absen.index') }}" class="text-xs text-emerald-700 hover:underline font-semibold flex items-center gap-1">
    ...
    @if(isset($absens) && $absens->hasPages())
      <div class="p-4 border-t border-slate-100 bg-slate-50/50 flex items-center justify-between">
        {{ $absens->withQueryString()->links('components.pagination') }}
      </div>
    @endif
  </div>
@endsection